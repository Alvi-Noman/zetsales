...
function agingTone(minutes: number): { text: string; ring: string } {
  if (minutes > 120)
    return { text: "text-rose-600", ring: "ring-rose-600/20 bg-rose-50" };
  if (minutes > 60)
    return { text: "text-amber-600", ring: "ring-amber-600/20 bg-amber-50" };
    ...
    <div className="rounded-lg border border-slate-200 bg-white">
      ...
      {queue.length === 0 ? (
        <div className="p-8 text-center text-sm text-slate-400">
          Queue is empty — every order has been called.
        </div>
      ) : (
        <div className="max-h-[420px] zs-table-body overflow-y-auto">
          {queue.map((item) => {
              ...
              >
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-0.5 flex items-center gap-2 text-[11px] text-slate-400">
                    ...
                    <span>
                      {item.callAttempts} call
                      {item.callAttempts === 1 ? "" : "s"}
                    </span>
                    {item.lastOutcome && (
                      <span className="truncate">· {item.lastOutcome}</span>
                    )}
                  </div>
                </div>
                <div className="shrink-0 text-right">
                  ...
                  <span
                    className={clsx(
                      "mt-0.5 inline-flex items-center gap-1 rounded-full px-1.5 py-0.5 text-[10px] font-semibold ring-1 ring-inset",
                      tone.text,
                      tone.ring,
                    )}
                  >
