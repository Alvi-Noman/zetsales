...
export function HourlyVolumeChart({
  ...
  data: CallCenterHourlyVolumeDTO[];
}) {
              ...
              >
                {hovered === d.hour && (
                  <div className="pointer-events-none absolute -top-11 left-1/2 z-10 -translate-x-1/2 whitespace-nowrap rounded-lg bg-slate-900 px-2.5 py-1.5 text-[11px] font-medium text-white shadow-lg">
                    <span className="font-semibold">{d.label}</span> — {d.calls}{" "}
                    call{d.calls === 1 ? "" : "s"}, {d.confirmed} confirmed
                  </div>
                )}
                <div
                  className="w-full max-w-[10px] rounded-t bg-indigo-400/80"
                  style={{ height: `${Math.max(2, (d.calls / max) * 100)}%` }}
                />
                <div
                  className="w-full max-w-[10px] rounded-t bg-emerald-500"
                  style={{
                    height: `${Math.max(d.confirmed > 0 ? 2 : 0, (d.confirmed / max) * 100)}%`,
                  }}
                />
              </div>
            ))}
