...
const BUSINESS_TYPE_OPTIONS: {
  ...
  {
    value: "I manufacture my own products",
    label: "I manufacture my own products",
    icon: Factory,
  },
  { value: "I import my products", label: "I import my products", icon: Ship },
  {
    value: "I buy from local wholesalers",
    label: "I buy from local wholesalers",
    icon: Store,
  },
  {
    value: "I dropship — I never hold stock",
    label: "I dropship — I never hold stock",
    icon: Truck,
  },
];
...
function OptionCard({
  ...
  return (
    <button
      onClick={onClick}
      className={clsx(
        "flex flex-col items-center gap-2.5 rounded-lg border p-5 text-center transition-colors",
        active
...
function ListOption({
  ...
  return (
    <button
      onClick={onClick}
      className={clsx(
        "flex w-full items-center justify-between rounded-lg border px-5 py-4 text-left text-sm font-medium transition-colors",
        active
  ...
  if (finishing) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4 bg-white">
        <div className="flex h-10 w-10 animate-pulse items-center justify-center rounded-lg bg-gradient-to-tr from-indigo-500 to-violet-500 text-sm font-bold text-white">
          Z
        </div>
      ...
      <div className="relative flex flex-1 items-center justify-center px-6 pb-24">
        ...
        >
          ...
          {step === "businessType" && (
            <div>
              <h1 className="text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl">
                How does {businessLabel} get its stock?
              </h1>
              ...
              <div className="mt-9 grid grid-cols-1 gap-3 sm:grid-cols-2">
                {BUSINESS_TYPE_OPTIONS.map((opt) => (
                  <OptionCard
                    ...
                    onClick={() =>
                      selectAndAdvance(() => setBusinessType(opt.value))
                    }
                    icon={opt.icon}
                    label={opt.label}
                  />
          ...
          {step === "phone" && (
            ...
            >
              ...
              <p className="mt-2.5 text-slate-500">
                For account and delivery-related updates — WhatsApp preferred.
              </p>
              <input
                autoFocus
                value={phone}
          ...
          {step === "channels" && (
            <div>
              <h1 className="text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl">
                Where does {businessLabel} sell today?
              </h1>
              <p className="mt-2.5 text-slate-500">
                Select every channel — you can connect them later.
              </p>
            ...
            <div>
              ...
              <div className="mt-9 space-y-2.5">
                {MONTHLY_ORDERS_OPTIONS.map((opt) => (
                  <ListOption
                    key={opt}
                    active={monthlyOrders === opt}
                    onClick={() =>
                      selectAndAdvance(() => setMonthlyOrders(opt))
                    }
                    label={opt}
                  />
                ))}
