            ...
            <>
              {showsLocation && warehouses.length > 0 && (
                <>
                  ...
                  {usesBins && <BinPicker value={bin} onChange={setBin} options={binOptions} placeholder="Shelf/Bin" compact />}
                </>
              )}
              <button
                onClick={() => void submit()}
...
export function ReturnsPage() {
  ...
  useEffect(() => {
    ...
    void getInventorySettings().then((res) => setReturnsWorkflow(res.settings.returnsWorkflow));
    ...
    <div className="flex min-h-screen flex-col bg-slate-50">
      <div className="border-b border-slate-200 bg-white px-4 py-4 lg:px-8">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h1 className="text-lg font-bold text-slate-900">Returns to process</h1>
            <p className="mt-0.5 text-sm text-slate-500">Courier-returned packages waiting on receipt and QC.</p>
          </div>
                ...
                {awaitingQc.length > 0 && (
                  <section className="rounded-xl border border-slate-200 p-4">
                    ...
                    <div className="max-h-[420px] space-y-2 overflow-y-auto pr-1">
                      {awaitingQc.map((pkg) => (
                        <QcPackageCard key={pkg.orderId} pkg={pkg} onDone={() => void loadReturnsQueue()} onLogFoundStock={logAsFoundStock} />
                      ))}
                    </div>
                  </section>
                )}
