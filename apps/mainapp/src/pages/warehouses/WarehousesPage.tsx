...
function AddWarehouseModal({
  ...
  const submit = async () => {
    ...
    try {
      ...
      toast.push(
        (err as Error).message || "Could not add this warehouse.",
        "info",
      );
    } finally {
      setSaving(false);
  ...
  return (
    <Modal
      ...
      onClose={close}
      title="Add warehouse"
      subtitle="Add a real location. Bins can be set up afterward, from the warehouse list."
    >
...
export function WarehousesPage() {
  ...
  const [newBinByWarehouse, setNewBinByWarehouse] = useState<
      ...
      const results = await Promise.all(
        list.map(async (w) => {
          ...
          return [
            w.id,
            {
              onHand: res.summary.onHand,
              inbound: res.summary.inbound,
  ...
  const saveRename = async (id: string) => {
    ...
    try {
      ...
      toast.push(
        (err as Error).message || "Could not rename this warehouse.",
        "info",
      );
    }
  };
  ...
  const remove = async (id: string) => {
    ...
    try {
      ...
      if (!res.success)
        toast.push(res.message || "Could not delete this warehouse.", "info");
      else void load();
    } catch (err) {
      toast.push(
        (err as Error).message || "Could not delete this warehouse.",
        "info",
      );
    } finally {
      setBusyId(null);
  ...
  const removeBin = async (id: string, name: string) => {
    try {
      ...
      toast.push(
        (err as Error).message || "Could not remove this bin.",
        "info",
      );
    }
  };
  ...
  const totals = useMemo(() => {
    ...
    return {
      ...
      binCount: warehouses.reduce(
        (sum, w) => sum + w.bins.length + w.systemBins.length,
        0,
      ),
        ...
        <div className="zs-toolbox-row">
          <div className="zs-toolbox-left">
            <span className="inline-flex h-8 items-center rounded-lg border border-slate-200 bg-slate-50 px-3 text-xs font-semibold text-slate-600">
              Locations {totals.warehouseCount.toLocaleString()}
            </span>
            <span className="inline-flex h-8 items-center rounded-lg border border-slate-200 bg-white px-3 text-xs font-semibold text-slate-500">
              Bins {totals.binCount.toLocaleString()}
            </span>
          </div>
      ...
      <div className="zs-page-body space-y-4">
        <div className="zs-summary-strip">
          <MetricCard
            icon={Building2}
            label="Warehouses"
            value={totals.warehouseCount.toLocaleString()}
            detail={`${totals.warehouseCount === 1 ? "location" : "locations"}`}
          />
          <MetricCard
            icon={MapPin}
            label="Bins"
            value={totals.binCount.toLocaleString()}
            detail="shelf locations tracked"
          />
          <MetricCard
            icon={Boxes}
            label="On hand"
            value={totals.onHand.toLocaleString()}
            detail="units across all warehouses"
            tone="indigo"
          />
          <MetricCard
            icon={Wallet}
            ...
            detail="at cost, across all warehouses"
            tone="emerald"
          />
        </div>
        ...
        <section className="zs-surface overflow-hidden">
          {loading ? (
            <div className="zs-loading-state">Loading warehouses...</div>
            ...
            <div className="zs-empty-state">
              ...
              <p className="max-w-md text-sm text-slate-400">
                {warehouses.length === 0
                  ? "Add your first real location above to start tracking stock by warehouse."
            ...
            <div className="zs-table-body">
              {filteredWarehouses.map((w) => {
                const s = stats[w.id];
                return (
                  <div key={w.id} className="zs-data-row p-4">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                        ...
                        <div className="flex items-center gap-2">
                          {editingId === w.id ? (
                            <input
                              ...
                              onKeyDown={(e) =>
                                e.key === "Enter" && void saveRename(w.id)
                              }
                        ...
                        <div className="mt-3 flex flex-wrap items-center gap-1.5">
                          {w.bins.map((bin) => (
                            ...
                            >
                              {bin}
                              <button
                                onClick={() => void removeBin(w.id, bin)}
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setNewBinByWarehouse((prev) => ({
                                ...prev,
                                [w.id]: e.target.value,
                              }))
                            }
                            onKeyDown={(e) =>
                              e.key === "Enter" && void addBin(w.id)
                            }
                      ...
                      <button
                        ...
                        title={
                          s && s.onHand + s.inbound > 0
                            ? "This warehouse still has stock — removing it may not be allowed."
                            : "Remove this warehouse"
                        }
                        className={clsx(
