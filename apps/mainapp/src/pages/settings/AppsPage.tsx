import { PhoneCall, Headset, Megaphone, ShieldAlert, Rocket, Blocks, Eye, type LucideIcon } from 'lucide-react';
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="border-b border-slate-200 bg-white px-8 py-5">
        ...
        <p className="mt-0.5 text-sm text-slate-500">Install optional plugins to extend your workspace.</p>
      </div>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {(apps ?? []).map(({ manifest, install }) => {
              ...
              return (
                <article
                  key={manifest.key}
                  className="flex min-h-[150px] flex-col rounded-xl border border-slate-200 bg-white p-5 text-left"
                >
