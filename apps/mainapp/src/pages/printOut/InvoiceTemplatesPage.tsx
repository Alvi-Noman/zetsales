...
function toForm(t: InvoiceTemplateDTO): PrintTemplateFields {
  return {
    name: t.name,
    logoUrl: t.logoUrl ?? "",
    businessNameOverride: t.businessNameOverride ?? "",
    address: t.address ?? "",
    phone: t.phone ?? "",
    paperSize: t.paperSize,
    showItemImages: t.showItemImages,
    showSkuVariant: t.showSkuVariant,
    showCustomerAddress: t.showCustomerAddress,
    showPaymentBox: t.showPaymentBox,
    showDeliveryBox: t.showDeliveryBox,
    showBarcode: t.showBarcode,
    showCodCallout: t.showCodCallout,
...
export function InvoiceTemplatesPage() {
  ...
  const save = async () => {
    ...
    try {
      ...
      toast.push(
        (err as Error).message || "Could not save this template.",
        "info",
      );
    } finally {
      setSaving(false);
  ...
  const previewTemplate: InvoiceTemplateDTO = {
    ...
    businessNameOverride: form.businessNameOverride || null,
    address: form.address || null,
    phone: form.phone || null,
    paperSize: form.paperSize,
    showItemImages: form.showItemImages,
    showSkuVariant: form.showSkuVariant,
    showCustomerAddress: form.showCustomerAddress,
    showPaymentBox: form.showPaymentBox,
    showDeliveryBox: form.showDeliveryBox,
    showBarcode: form.showBarcode,
    showCodCallout: form.showCodCallout,
  ...
  return (
    <div className="zs-page">
      <div className="zs-page-header flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="zs-page-title">Invoice Design</h1>
          <p className="zs-page-description">
            Save as many templates as you need — logo, layout, what shows on the
            page.
          </p>
        </div>
        {!editorOpen && (
      ...
      <div className="zs-page-body">
        {editorOpen ? (
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-[380px_minmax(0,1fr)]">
            <section className="space-y-4 zs-surface p-4">
              <div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setForm((p) => ({
                      ...p,
                      businessNameOverride: e.target.value,
                    }))
                  }
              ...
              <div>
                ...
                <Select
                  ...
                  onChange={(v) =>
                    setForm((p) => ({ ...p, paperSize: v as PrintPaperSize }))
                  }
                  options={[
                    { value: "A4", label: "A4" },
                    { value: "A5", label: "A5" },
              ...
              <div className="space-y-1.5">
                <p className="text-xs font-semibold text-slate-600">
                  What shows on the document
                </p>
                <Toggle
                  label="Item images"
                  checked={form.showItemImages}
                  onChange={(v) =>
                    setForm((p) => ({ ...p, showItemImages: v }))
                  }
                />
                <Toggle
                  label="Variant / SKU"
                  checked={form.showSkuVariant}
                  onChange={(v) =>
                    setForm((p) => ({ ...p, showSkuVariant: v }))
                  }
                />
                <Toggle
                  label="Customer address"
                  checked={form.showCustomerAddress}
                  onChange={(v) =>
                    setForm((p) => ({ ...p, showCustomerAddress: v }))
                  }
                />
                <Toggle
                  label="Payment box"
                  checked={form.showPaymentBox}
                  onChange={(v) =>
                    setForm((p) => ({ ...p, showPaymentBox: v }))
                  }
                />
                <Toggle
                  label="Delivery box"
                  checked={form.showDeliveryBox}
                  onChange={(v) =>
                    setForm((p) => ({ ...p, showDeliveryBox: v }))
                  }
                />
                <Toggle
                  label="Barcode"
                  checked={form.showBarcode}
                  onChange={(v) => setForm((p) => ({ ...p, showBarcode: v }))}
                />
                <Toggle
                  label="COD collect box"
                  checked={form.showCodCallout}
                  onChange={(v) =>
                    setForm((p) => ({ ...p, showCodCallout: v }))
                  }
                />
              </div>
              ...
              <div className="flex gap-2 pt-1">
                <button
                  ...
                  className="flex-1 rounded-lg px-3.5 py-2 text-sm font-semibold text-slate-500 hover:bg-slate-50"
                >
                  Cancel
                </button>
                <button
            ...
            <section className="zs-surface overflow-hidden bg-slate-100 p-4">
          ...
          <div className="zs-loading-state">Loading templates...</div>
        ) : templates.length === 0 ? (
          <div className="zs-empty-state zs-surface">
            <ImagePlus size={28} className="text-slate-300" />
            <p className="text-sm font-semibold text-slate-700">
              No templates yet
            </p>
            <p className="max-w-md text-sm text-slate-400">
              Create one to add your logo and control what shows on invoices and
              packing slips.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3">
            {templates.map((t) => (
              <div key={t.id} className="zs-surface p-4">
                  ...
                  <div className="flex items-center gap-2.5">
                    {t.logoUrl ? (
                      <img
                        src={t.logoUrl}
                        alt=""
                        className="h-9 w-9 rounded-lg border border-slate-100 object-contain"
                      />
                    ) : (
                ...
                <div className="mt-3 flex items-center gap-2">
                  <button
                    onClick={() => openEdit(t)}
                    className="flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-slate-200 px-2.5 py-1.5 text-xs font-semibold text-slate-600 hover:bg-slate-50"
                  >
                    <Pencil size={12} /> Edit
                  </button>
                  {!t.isDefault && (
