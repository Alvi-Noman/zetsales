...
export function TeamPage() {
  ...
  const grantableRoles = useMemo<TeamRole[]>(
    () =>
      myRole === "owner"
  ...
  const load = async () => {
    ...
    try {
      const [memberList, inviteList] = await Promise.all([
        listMembers(),
        canManageTeam ? listInvites() : Promise.resolve([]),
      ]);
  ...
  const canManageTarget = (member: TeamMemberDTO) =>
    canManageTeam &&
    !member.isYou &&
    member.role !== "owner" &&
  ...
  return (
    <div className="zs-page-scroll">
      ...
      <div className="zs-page-body space-y-8">
        <div>
            ...
            <div className="zs-table-wrap">
              <div className="zs-table-body">
                {members.map((member) => (
                  <div
                    key={member.id}
                    className="zs-data-row flex items-center justify-between gap-4 px-4 py-3"
                  >
        ...
        <div>
          <h2 className="mb-3 flex items-center gap-1.5 text-sm font-semibold text-slate-700">
            <ShieldCheck size={15} className="text-slate-400" /> Roles &
            permissions
          </h2>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {(
              Object.values(
                ROLE_DEFINITIONS,
              ) as (typeof ROLE_DEFINITIONS)[TeamRole][]
            ).map((def) => (
              <div key={def.role} className="zs-surface p-4">
                ...
                <p className="mt-2 text-[11px] text-slate-400">
                  {def.modules.map((m) => MODULE_LABELS[m]).join(", ")}
                </p>
              </div>
            ))}
          </div>
