...
type RangePreset = 'thisMonth' | 'lastMonth' | 'thisQuarter' | 'thisYear' | 'custom';
...
const EXPENSE_CATEGORY_META: { category: string; description: string; icon: typeof Megaphone }[] = [
  { category: 'Advertising', description: 'Facebook/Google ads, boosted posts, promotions', icon: Megaphone },
  { category: 'Payroll', description: 'Staff salaries and wages', icon: Users },
  { category: 'Rent & Utilities', description: 'Warehouse or office rent, electricity, internet', icon: Landmark },
  { category: 'Software & Tools', description: 'Subscriptions, courier/API fees, apps', icon: Wrench },
  { category: 'Other', description: "Anything that doesn't fit the above", icon: Wallet },
];

function ExpenseCategoryPicker({ value, onChange }: { value: string; onChange: (category: string) => void }) {
    ...
    <Popover
      ...
      trigger={() => (
        <div className="flex h-10 w-full cursor-pointer items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 text-sm text-slate-800 hover:bg-slate-50">
          {preset ? <preset.icon size={14} className="shrink-0 text-slate-400" /> : <Wallet size={14} className="shrink-0 text-slate-400" />}
          <span className="flex-1 truncate">{value || 'Select a category'}</span>
            ...
            return (
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  <span className={clsx('block text-xs font-semibold', selected ? 'text-indigo-700' : 'text-slate-700')}>{option.category}</span>
...
function AddExpenseModal({ open, onClose, onSaved }: { open: boolean; onClose: () => void; onSaved: () => void }) {
  ...
  const canSave = category.trim().length > 0 && amount.trim() !== '' && Number(amount) > 0 && date.trim().length > 0 && !saving;

  const save = async () => {
    if (!canSave) return;
    setSaving(true);
    try {
      const payload: ExpensePayload = { category: category.trim(), amount: Number(amount), date, note: note.trim() || undefined };
      await createExpense(payload);
      toast.push('Expense logged.', 'success');
      onSaved();
      onClose();
    } catch (err) {
      toast.push((err as Error).message || 'Could not save this expense.', 'info');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Modal open={open} onClose={onClose} title="Add expense" subtitle="Advertising, payroll, office costs — anything outside inventory." widthClass="max-w-md">
...
function TrendChart({ trend }: { trend: ProfitAndLossDTO['trend'] }) {
  ...
  return (
    <div>
      <div className="flex items-end gap-3 border-b border-slate-100 pb-2" style={{ height: 180 }}>
        {trend.map((t) => (
          <div key={t.month} className="flex flex-1 items-end justify-center gap-0.5" title={`${monthLabel(t.month)}: revenue ${money(t.revenue)}, cost ${money(t.cogs)}, profit ${money(t.profit)}`}>
            <div className="w-2.5 rounded-t bg-emerald-400" style={{ height: `${(t.revenue / max) * 100}%` }} />
            <div className="w-2.5 rounded-t bg-rose-300" style={{ height: `${(t.cogs / max) * 100}%` }} />
          </div>
        ))}
      </div>
...
export function AccountingPage() {
  ...
  const [committed, setCommitted] = useState<CommittedToSuppliersDTO | null>(null);
  ...
  const load = async () => {
    ...
    try {
      ...
      const [pnlRes, expensesRes] = await Promise.all([getProfitAndLoss(params), listExpenses(params)]);
      setPnl(pnlRes);
  ...
  const removeExpense = async (id: string) => {
    try {
      ...
      toast.push((err as Error).message || 'Could not remove this expense.', 'info');
    }
  };
  ...
  return (
    <div className="flex h-full flex-col bg-white">
      <div className="flex items-center justify-between border-b border-slate-200 bg-white px-8 py-5">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="rounded-full bg-indigo-50 px-2 py-0.5 text-[11px] font-semibold text-indigo-600">Profit &amp; Loss</span>
          </div>
          <p className="mt-1 text-sm text-slate-500">Revenue, cost of goods sold, inventory loss, and operating expenses.</p>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={() => void load()}
      ...
      <div className="flex-1 overflow-y-auto px-8 py-6">
        {loading && !pnl ? (
          <div className="flex h-64 items-center justify-center text-sm text-slate-400">Loading Profit &amp; Loss...</div>
        ) : pnl ? (
          <div className="space-y-6">
            <div className="flex flex-wrap rounded-xl border border-slate-200 bg-white">
              <MetricCard icon={TrendingUp} label="Revenue" value={money(pnl.revenue)} tone="emerald" />
            ...
            <div className="grid grid-cols-2 gap-4">
              <section className="rounded-xl border border-slate-200 p-5">
                <h2 className="mb-4 text-sm font-bold text-slate-900">Income statement</h2>
                <div className="space-y-2.5 text-sm">
                  <div className="flex items-center justify-between">
                    <span className="text-slate-600">Revenue</span>
                    <span className="font-semibold tabular-nums text-slate-800">{money(pnl.revenue)}</span>
                  </div>
                  <p className="text-[11px] text-slate-400">Reflects delivered order value; does not yet net out refunds from later returns.</p>
                  <div className="flex items-center justify-between border-t border-slate-100 pt-2.5">
                    <span className="text-slate-600">Cost of goods sold</span>
                    <span className="font-semibold tabular-nums text-rose-600">-{money(pnl.cogs)}</span>
                  </div>
                  <div className="flex items-center justify-between border-t border-slate-200 pt-2.5">
                    <span className="font-bold text-slate-900">Gross profit</span>
                    <span className="font-bold tabular-nums text-slate-900">{money(pnl.grossProfit)}</span>
                  </div>
                  <div className="flex items-center justify-between border-t border-slate-100 pt-2.5">
                  ...
                  {pnl.expensesByCategory.length === 0 ? (
                    ...
                    pnl.expensesByCategory.map((e) => (
                      <div key={e.category} className="flex items-center justify-between border-t border-slate-100 pt-2.5 pl-3">
                        <span className="text-slate-500">{e.category}</span>
                        <span className="font-medium tabular-nums text-rose-500">-{money(e.amount)}</span>
                      </div>
                    ))
                  )}
                  <div className="flex items-center justify-between border-t-2 border-slate-800 pt-2.5">
                    ...
                    <span className={clsx('font-bold tabular-nums', pnl.netProfit >= 0 ? 'text-emerald-600' : 'text-rose-600')}>{money(pnl.netProfit)}</span>
                  </div>
                </div>
              </section>

              <section className="rounded-xl border border-slate-200 p-5">
                <h2 className="mb-4 text-sm font-bold text-slate-900">Revenue vs. cost — trailing 12 months</h2>
            ...
            <section className="rounded-xl border border-dashed border-slate-300 bg-slate-50/50 p-5">
              <div className="mb-1 flex items-center gap-2">
                <h2 className="text-sm font-bold text-slate-900">Committed to suppliers</h2>
              ...
              <p className="mb-4 text-xs text-slate-500">
                A live snapshot of open shipments right now — not a range like the numbers above. Split by how you pay each supplier: cash
                already spent and sitting in transit, versus money you genuinely still owe.
              </p>
              {committedLoading || !committed ? (
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="rounded-lg border border-slate-200 bg-white p-4">
                    ...
                    <p className="mt-1.5 text-xl font-bold tabular-nums text-slate-900">{money(committed.prepaidInTransit.total)}</p>
                    <p className="mt-1 text-xs text-slate-400">
                      {committed.prepaidInTransit.shipmentCount} open shipment{committed.prepaidInTransit.shipmentCount === 1 ? '' : 's'}
                      {committed.prepaidInTransit.overdueShipmentCount > 0 && (
                        <span className="text-rose-600">
                          {' '}
                          · {money(committed.prepaidInTransit.overdueTotal)} overdue ({committed.prepaidInTransit.overdueShipmentCount})
                        </span>
                      )}
                    </p>
                  </div>
                  <div className="rounded-lg border border-slate-200 bg-white p-4">
                    ...
                    <p className="mt-1.5 text-xl font-bold tabular-nums text-slate-900">{money(committed.owedToSuppliers.total)}</p>
                    <p className="mt-1 text-xs text-slate-400">
                      {committed.owedToSuppliers.shipmentCount} open shipment{committed.owedToSuppliers.shipmentCount === 1 ? '' : 's'}
                      {committed.owedToSuppliers.overdueShipmentCount > 0 && (
                        <span className="text-rose-600">
                          {' '}
                          · {money(committed.owedToSuppliers.overdueTotal)} overdue ({committed.owedToSuppliers.overdueShipmentCount})
                        </span>
                      )}
                    </p>
                  </div>
              ...
              {committed && committed.bySupplier.length > 0 && (
                <div className="mt-4 divide-y divide-slate-100 border-t border-slate-100 pt-1">
                  {committed.bySupplier.map((s) => (
                    <div key={s.supplierId ?? s.supplierName} className="flex items-center justify-between gap-3 py-2 text-sm">
                      <div className="flex min-w-0 items-center gap-2">
                        <span className="truncate font-medium text-slate-700">{s.supplierName}</span>
                        <span
                          className={clsx(
                            'shrink-0 rounded-full px-1.5 py-0.5 text-[10px] font-semibold',
                            s.paymentType === 'credit' ? 'bg-amber-50 text-amber-700' : 'bg-emerald-50 text-emerald-700'
                          )}
                        >
                      ...
                      <div className="flex shrink-0 items-center gap-2 text-xs text-slate-500">
                        ...
                        <span className="font-bold tabular-nums text-slate-800">{money(s.total)}</span>
                        {s.overdueTotal > 0 && <Clock size={12} className="text-rose-500" />}
                      </div>
                    </div>
                  ))}
                </div>
              )}
              {committed && committed.bySupplier.length === 0 && !committedLoading && <p className="py-4 text-center text-xs text-slate-400">No open shipments right now.</p>}
            </section>

            <section className="rounded-xl border border-slate-200 p-4">
              <div className="mb-3 flex items-center justify-between">
                <h2 className="text-sm font-bold text-slate-900">Expenses in this range</h2>
                <span className="text-xs text-slate-400">{expenses.length} entr{expenses.length === 1 ? 'y' : 'ies'}</span>
              </div>
              {expenses.length === 0 ? (
                <p className="py-6 text-center text-sm text-slate-400">No expenses logged for this range yet — advertising, payroll, and office costs will show up here once added.</p>
              ) : (
                <div className="divide-y divide-slate-100">
                  {expenses.map((e) => (
                    <div key={e.id} className="flex items-center justify-between gap-3 py-2.5">
                      <div className="min-w-0">
                        <p className="truncate text-sm font-medium text-slate-700">{e.category}</p>
                      ...
                      <div className="flex shrink-0 items-center gap-3">
                        <span className="font-bold tabular-nums text-slate-800">{money(e.amount)}</span>
                        <button onClick={() => void removeExpense(e.id)} className="rounded-md p-1.5 text-slate-300 hover:bg-rose-50 hover:text-rose-500">
                          <Trash2 size={14} />
                        </button>
                      </div>
                    </div>
      ...
      <AddExpenseModal open={addExpenseOpen} onClose={() => setAddExpenseOpen(false)} onSaved={() => void load()} />
    </div>
  );
}
