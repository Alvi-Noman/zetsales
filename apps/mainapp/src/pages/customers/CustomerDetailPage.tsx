...
function MetricCard({ icon: Icon, label, value, tone = 'slate' }: { icon: typeof Package; label: string; value: string; tone?: 'slate' | 'emerald' | 'indigo' }) {
...
export function CustomerDetailPage() {
  ...
  const load = async () => {
    if (!phone) return;
    try {
      const [customerRes, ordersRes] = await Promise.all([getCustomer(phone), listCustomerOrders(phone)]);
  ...
  const handleToggleBlock = async () => {
    ...
    try {
      if (customer.isBlocked) {
        ...
        toast.push('Customer blocked — future orders from this number auto-cancel.');
      }
      setCustomer((prev) => (prev ? { ...prev, isBlocked: !prev.isBlocked } : prev));
    } catch {
  ...
  return (
    <div className="flex h-full flex-col overflow-y-auto bg-slate-50">
      <div className="border-b border-slate-200 bg-white px-8 py-5">
        <button onClick={() => navigate('/customers')} className="mb-3 flex items-center gap-1.5 text-xs font-medium text-slate-400 hover:text-slate-700">
          <ArrowLeft size={13} /> All customers
        </button>
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-lg font-bold text-slate-900">{customer.name ?? 'No name'}</h1>
              <span className={clsx('rounded-full px-2 py-0.5 text-xs font-medium ring-1 ring-inset', RISK_TONE[customer.riskLabel])}>{customer.riskLabel}</span>
              <span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-600">{customer.segment}</span>
            </div>
            <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-sm text-slate-500">
              ...
              <span className="flex items-center gap-1.5">
                <Calendar size={13} /> Customer since {new Date(customer.firstOrderAt).toLocaleDateString()}
              </span>
            </div>
          </div>
          <button
            ...
            className={clsx(
              ...
              customer.isBlocked ? 'border border-emerald-200 text-emerald-700 hover:bg-emerald-50' : 'border border-rose-200 text-rose-600 hover:bg-rose-50'
            )}
          >
      ...
      <div className="flex-1 space-y-5 px-8 py-6">
        <div className="flex flex-wrap rounded-xl border border-slate-200 bg-white">
          <MetricCard icon={Package} label="Orders" value={String(customer.orderCount)} tone="slate" />
          <MetricCard icon={Wallet} label="Lifetime spend" value={money(customer.totalSpend)} tone="indigo" />
        ...
        <div>
          ...
          {orders === null ? (
            <div className="flex h-32 items-center justify-center text-sm text-slate-400">Loading orders...</div>
          ) : (
