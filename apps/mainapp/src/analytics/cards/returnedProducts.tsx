...
import { getReturnedProducts } from '../../lib/analyticsApi';
...
function Card({ query }: AnalyticsCardComponentProps) {
  ...
  return (
    <AnalyticsCardShell title="Top returned products" cardKey="returnedProducts" loading={!data} headlineValue={data ? formatCount(data.rows.length) : undefined} trendGoodDirection="down">
      {data && (
        <div className="space-y-2">
          {data.rows.slice(0, 4).map((r) => (
            <div key={r.productId} className="flex items-center justify-between gap-2 text-[12px]">
              ...
              <span className="shrink-0 font-semibold tabular-nums text-slate-800">{formatCount(r.unitsSold)}</span>
            </div>
          ))}
          {data.rows.length === 0 && <p className="text-xs text-slate-300">No returns/RTO in this period</p>}
        </div>
      )}
    </AnalyticsCardShell>
  );
...
function Detail({ query }: AnalyticsCardComponentProps) {
  ...
  return (
    <RankedTable
      ...
      rows={data.rows}
      emptyLabel="No returns/RTO in this period"
      columns={[
        {
          key: 'title',
          header: 'Product',
        ...
        { key: 'units', header: 'Units returned/RTO', align: 'right', render: (r) => formatCount(r.unitsSold) },
        { key: 'orders', header: 'Orders', align: 'right', render: (r) => formatCount(r.orderCount) },
        { key: 'value', header: 'Value affected', align: 'right', render: (r) => formatMoney(r.revenue) },
        { key: 'rate', header: 'Return rate', align: 'right', render: (r) => (r.returnRate != null ? <span className="font-semibold text-slate-900">{formatPercent(r.returnRate)}</span> : '—') },
      ]}
    />
  );
}

export const returnedProductsCard: AnalyticsCardDefinition = {
  key: 'returnedProducts',
  title: 'Top returned products',
  category: 'Inventory',
  description: 'Which products come back most — via return, or RTO on the way out.',
  icon: RotateCcw,
  CardComponent: Card,
