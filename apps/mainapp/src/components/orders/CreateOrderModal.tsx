...
export function CreateOrderModal({
  ...
  const [courierSpeed, setCourierSpeed] = useState<"regular" | "express">(
    "regular",
  );
  ...
  const handlePickProduct = async (product: PickedProduct) => {
    ...
    try {
      ...
      const opts = res.product.variants.map((v) => ({
        id: v.id,
        label:
  ...
  const addItem = () => {
    ...
    setItems((prev) => [
      ...prev,
      {
        ...
        variantId,
        title: picked.title,
        variantLabel: variant.label === "Default" ? null : variant.label,
        price: variant.price,
        quantity: qty,
      },
    ]);
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      title="Create order"
      subtitle="Log a phone or walk-in order that didn't come through Shopify/WooCommerce."
      widthClass="max-w-lg"
    >
      <div className="space-y-4">
        ...
        <div className="rounded-lg border border-slate-200 p-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-400">
                        {li.variantLabel ? `${li.variantLabel} · ` : ""}Qty{" "}
                        {li.quantity}
                      </p>
                    </div>
                  </div>
          ...
          <div className="space-y-2">
            <ProductPicker
              value={picked}
              onChange={(p) => void handlePickProduct(p)}
