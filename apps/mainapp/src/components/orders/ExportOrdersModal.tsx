...
function RadioRow({ checked, disabled, label, sublabel, onSelect }: RadioRowProps) {
  return (
    <label className={clsx('flex items-center gap-2.5 py-1.5', disabled ? 'cursor-not-allowed opacity-40' : 'cursor-pointer')}>
      <input
        type="radio"
        checked={checked}
        disabled={disabled}
...
export function ExportOrdersModal({ open, onClose, pageCount, filteredCount, selectedCount, hasActiveFilters, exporting, onExport }: ExportOrdersModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 px-4" onClick={onClose}>
      <div className="w-full max-w-sm rounded-xl bg-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-5 py-4">
          <p className="mb-1 text-xs font-semibold uppercase tracking-wide text-slate-400">Export</p>
          <RadioRow checked={scope === 'page'} label="Current page" sublabel={`(${pageCount} orders)`} onSelect={() => setScope('page')} />
          <RadioRow
            checked={scope === 'filtered'}
            label={hasActiveFilters ? 'All orders matching filters' : 'All orders'}
          ...
          <p className="mb-1 mt-4 text-xs font-semibold uppercase tracking-wide text-slate-400">Export as</p>
          <RadioRow checked={format === 'excel'} label="CSV for Excel, Numbers, or other spreadsheet programs" onSelect={() => setFormat('excel')} />
          <RadioRow checked={format === 'plain'} label="Plain CSV file" onSelect={() => setFormat('plain')} />
        </div>

        <div className="flex items-center justify-end gap-2 border-t border-slate-100 px-5 py-4">
          <button onClick={onClose} className="rounded-lg border border-slate-200 px-3.5 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">
            Cancel
          </button>
          <button
