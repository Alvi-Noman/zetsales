...
export function CommandPalette({
  ...
  const actions = useMemo<QuickAction[]>(() => {
    const list: QuickAction[] = [
      ...
      },
      {
        ...
        hint: "Reset to default view",
        run: onClearFilters,
      },
      ...ORDER_TABS.map((tab) => ({
        id: `tab-${tab.key}`,
        label: `Go to "${tab.label}" tab`,
        run: () => onSelectTab(tab.key),
      })),
    ];
    if (selectedCount > 0) {
      list.unshift({
  ...
  const flatItems = useMemo(
    () => [
      ...results.map((o) => ({ type: "order" as const, order: o })),
  ...
  return (
    <div
      className="fixed inset-0 z-[200] flex items-start justify-center bg-slate-900/50 pt-[12vh]"
      onClick={onClose}
    >
      <div
