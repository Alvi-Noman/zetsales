...
function Thumb({ item, size, className }: ThumbProps) {
  return (
    <div
      className={clsx(
        "flex shrink-0 items-center justify-center overflow-hidden rounded-lg bg-slate-100 ring-2 ring-white",
        className,
      )}
    ...
    >
      {item?.image ? (
        ...
        <Package size={size * 0.45} className="text-slate-400" />
      )}
    </div>
  );
}
      ...
      trigger={() => (
        <div className="flex items-center gap-3 cursor-pointer">
          <div
            ...
            style={{ width: 24 + stack.length * 14, height: 36 }}
          >
            {stack.map((li, i) => (
              <div
                key={i}
                className="absolute top-0"
                style={{ left: i * 14, zIndex: stack.length - i }}
              >
          ...
          <div className="min-w-0 max-2xl:hidden">
            <p className="max-w-[170px] truncate font-medium text-slate-700">
              {first.title}
            </p>
    ...
    >
      <div className="max-h-96 overflow-y-auto p-3">
        <p className="px-1 pb-2.5 text-[10px] font-semibold uppercase tracking-wide text-slate-400">
          {lineItems.length} item{lineItems.length === 1 ? "" : "s"} in this
          order
        </p>
