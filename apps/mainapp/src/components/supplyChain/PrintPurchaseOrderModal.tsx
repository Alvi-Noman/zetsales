...
export function PrintPurchaseOrderModal({
  open,
  onClose,
  purchaseOrder,
}: PrintPurchaseOrderModalProps) {
  const { user } = useAuth();
            ...
            <div className="rounded-lg bg-white p-4 shadow-sm">
              ...
              {purchaseOrder.expectedAt && (
                <p className="text-slate-600">
                  Expected {formatDate(purchaseOrder.expectedAt)}
                </p>
              )}
            </div>
          </div>
          ...
          <div className="mt-4 ml-auto w-64 space-y-1.5 text-sm">
            ...
            {purchaseOrder.shippingTotal > 0 && (
              <div className="flex justify-between text-slate-500">
                <span>Shipping</span>
                <span className="tabular-nums">
                  {money(purchaseOrder.shippingTotal)}
                </span>
              </div>
            )}
            {purchaseOrder.dutiesTotal > 0 && (
              <div className="flex justify-between text-slate-500">
                <span>Duties</span>
                <span className="tabular-nums">
                  {money(purchaseOrder.dutiesTotal)}
                </span>
              </div>
            )}
