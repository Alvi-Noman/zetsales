    ...
    <div className="zs-table-wrap overflow-x-auto">
      <table className="w-full min-w-[480px] text-left text-[12.5px]">
        <thead>
          <tr className="zs-table-head">
            {columns.map((col) => (
              <th
                key={col.key}
                className={`px-3.5 py-2.5 font-semibold uppercase tracking-wide text-[10.5px] text-slate-400 ${col.align === "right" ? "text-right" : "text-left"}`}
              >
                {col.header}
              </th>
            ))}
        ...
        <tbody className="zs-table-body">
          {rows.map((row, i) => (
            <tr key={keyField(row)} className="zs-data-row">
              {columns.map((col) => (
                <td
                  key={col.key}
                  className={`px-3.5 py-2.5 ${col.align === "right" ? "text-right tabular-nums" : "text-left"}`}
                >
                  {col.render(row, i)}
                </td>
              ))}
