...
export function ProductDetailDrawer({
  ...
  const [detail, setDetail] = useState<{
    product: ProductDTO;
    ownStore: ProductStoreRef | null;
    siblings: ProductStoreRef[];
  } | null>(null);
  ...
  const channels = detail
    ? [...(detail.ownStore ? [detail.ownStore] : []), ...detail.siblings]
    : [];
          ...
          <>
            ...
            {loading || !product ? (
              ...
              <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
                <div className="flex items-center gap-4">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-slate-400">
                      ...
                      {product.variants.length === 1 ? "" : "s"} &middot;
                ...
                {channels.length > 0 && (
                  <section>
                    <h4 className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-400">
                      Listed on {channels.length} channel
                      {channels.length === 1 ? "" : "s"}
                    </h4>
                            ...
                            {v.compareAtPrice != null &&
                              v.compareAtPrice > v.price && (
                                <span className="ml-1.5 text-xs font-normal text-slate-400 line-through">
                                  ৳
                                  {v.compareAtPrice.toLocaleString(undefined, {
                                    minimumFractionDigits: 2,
                                  })}
                                </span>
                              )}
                ...
                <AppBlock
                  target="admin.product-details.block"
                  context={{ productId: product.id }}
                />

                {detail?.ownStore?.platform === "shopify" &&
                  (() => {
                    const ownStoreFull = stores.find(
                      (s) => s.id === detail.ownStore!.storeId,
                    );
                    return ownStoreFull ? (
                      <a
                        href={shopifyProductUrl(
                          ownStoreFull.shopDomain,
                          product.externalId,
                        )}
                        target="_blank"
                        rel="noreferrer"
