...
import { facebookOAuthStartUrl, getMessagingCapabilities, listSocialAccounts, removeSocialAccount } from '../../lib/messagingApi';
import { useToast } from '../ui/ToastProvider';

const PROVIDER_META = {
  facebook: { label: 'Facebook Page', color: 'bg-[#1877F2]', icon: MessageCircle },
  instagram: { label: 'Instagram', color: 'bg-gradient-to-br from-[#f09433] via-[#dc2743] to-[#bc1888]', icon: Camera },
} as const;

function AccountCard({ account, onRemove }: { account: SocialAccountDTO; onRemove: (account: SocialAccountDTO) => void }) {
      ...
      <div className="flex items-center gap-4">
        <span
          className={clsx(
            ...
            account.status === 'connected' ? 'bg-emerald-50 text-emerald-700 ring-emerald-600/20' : 'bg-rose-50 text-rose-700 ring-rose-600/20'
          )}
        >
          {account.status === 'connected' ? 'Connected' : 'Error'}
        </span>
        <button onClick={() => onRemove(account)} className="rounded-lg p-2 text-slate-400 hover:bg-rose-50 hover:text-rose-600">
          <Ban size={14} />
        </button>
      </div>
    </div>
...
export function MessagingIntegrationsTab() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [caps, list] = await Promise.all([getMessagingCapabilities(), listSocialAccounts()]);
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between rounded-xl border border-slate-200 bg-white p-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-bold text-slate-800">Facebook &amp; Instagram</p>
            <p className="text-xs text-slate-400">
              {accounts.length > 0 ? `${accounts.length} account${accounts.length === 1 ? '' : 's'} connected` : 'Not connected'}
            </p>
          </div>
        </div>
        <button
      ...
      {!metaAppConfigured && (
        <div className="rounded-lg border border-amber-200 bg-amber-50 px-3.5 py-2.5 text-sm text-amber-700">
          Facebook/Instagram messaging isn't configured on this server yet. Set META_APP_ID and META_APP_SECRET.
        </div>
      )}

      <div>
        <h2 className="mb-3 text-sm font-semibold text-slate-700">Connected pages &amp; accounts</h2>
        {loading ? (
          ...
          <div className="flex flex-col items-center gap-2 rounded-xl border border-dashed border-slate-300 py-14 text-center">
            ...
            <p className="text-sm font-medium text-slate-600">No pages connected yet</p>
            <p className="text-xs text-slate-400">
              Connect your Business Suite above to bring every Page's Messenger and Instagram DMs into one inbox.
            </p>
          </div>
        ) : (
          <div className="space-y-3">
            {accounts.map((account) => (
              <AccountCard key={account.id} account={account} onRemove={handleRemove} />
            ))}
          </div>
        )}
      </div>
