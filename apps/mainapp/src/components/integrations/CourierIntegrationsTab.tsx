...
function SettlementsSection({ courier }: { courier: CourierAccountDTO }) {
  ...
  const [settlements, setSettlements] = useState<CourierSettlementDTO[] | null>(null);
  ...
  const handleRecord = async () => {
    ...
    setSaving(true);
    try {
      await createCourierSettlement(courier.id, { amount: parsed, settledAt, note: note.trim() || undefined });
      setAmount('');
      setNote('');
      toast.push('Payout recorded.');
      void load();
    } catch (err) {
      toast.push(err instanceof Error ? err.message : 'Could not record this payout.', 'info');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="border-t border-slate-100 pt-3">
      <button onClick={() => setOpen((o) => !o)} className="flex w-full items-center justify-between text-left">
        <span className="text-xs font-semibold text-slate-600">Payouts received {settlements && `(${formatMoney(totalPaid)} total)`}</span>
        ...
        <div className="mt-3 space-y-3">
          ...
          <div className="space-y-1.5">
            {settlements === null ? (
              ...
              settlements.map((s) => (
                <div key={s.id} className="flex items-center justify-between rounded-md px-1.5 py-1 text-xs hover:bg-slate-50">
                  <span className="text-slate-600">
                    {new Date(s.settledAt).toLocaleDateString()} {s.note && <span className="text-slate-400">· {s.note}</span>}
                  </span>
                  <span className="flex items-center gap-2">
                    <span className="font-semibold text-slate-800">{formatMoney(s.amount)}</span>
                    <button onClick={() => handleDelete(s.id)} className="text-slate-300 hover:text-rose-500">
                      <Trash2 size={12} />
                    </button>
                  </span>
                </div>
        ...
        {orders === null ? (
          ...
          <>
            <div className="flex items-center justify-between rounded-lg bg-slate-50 px-3 py-2 text-xs text-slate-500">
              <button onClick={() => setSelected(new Set(orders.map((o) => o.orderId)))} className="font-semibold text-indigo-600 hover:text-indigo-700">
                Select all
              </button>
              <span>
                {selected.size} of {orders.length} selected · {totalItems} items · {formatMoney(totalCod)} COD
              </span>
            </div>
            <div className="max-h-64 overflow-y-auto rounded-lg border border-slate-200">
              {orders.map((o) => (
                <label key={o.orderId} className="flex cursor-pointer items-center gap-3 border-b border-slate-100 px-3 py-2 text-sm last:border-b-0 hover:bg-slate-50">
                  <input type="checkbox" checked={selected.has(o.orderId)} onChange={() => toggle(o.orderId)} className="rounded border-slate-300" />
                  <span className="flex-1 truncate">
                    #{o.orderNumber} <span className="text-slate-400">{o.customerName ?? 'No name'}</span>
                  </span>
                  <span className="shrink-0 text-slate-400">{o.itemCount} items</span>
                  <span className="w-20 shrink-0 text-right font-semibold text-slate-700">{formatMoney(o.total)}</span>
                </label>
              ))}
            </div>
          </>
...
function HandoverDetailModal({
  ...
  useEffect(() => {
    ...
    setDetail(null);
    void getCourierHandover(handoverId).then(({ handover }) => setDetail(handover));
  }, [handoverId]);
        ...
        <div id="handover-manifest-print" className="space-y-5">
          <div className="flex flex-wrap items-center gap-4 rounded-xl bg-slate-50 p-4">
            <div>
              ...
              >
                {detail.status === 'Confirmed' ? <CheckCircle2 size={11} /> : null}
                {detail.status}
              </span>
            </div>
            <div>
            ...
            <div>
              <p className="text-[10.5px] font-semibold uppercase tracking-wide text-slate-400">Total COD</p>
              <p className="text-sm font-bold text-slate-800">{formatMoney(detail.totalCodAmount)}</p>
            </div>
            {detail.note && (
          ...
          <div>
            <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-500">Orders in this batch</h3>
            <div className="overflow-x-auto rounded-lg border border-slate-200">
              <table className="w-full text-left text-xs">
                ...
                <tbody className="divide-y divide-slate-100">
                  {detail.orders.map((o) => (
                    <tr key={o.orderId}>
                      <td className="px-3 py-2 font-medium text-slate-700">#{o.orderNumber}</td>
                      <td className="px-3 py-2 text-slate-500">{o.customerName ?? '—'}</td>
                      <td className="px-3 py-2 text-slate-500">{o.consignmentId ?? '—'}</td>
                      <td className="px-3 py-2 text-right">{o.itemCount}</td>
                      <td className="px-3 py-2 text-right font-semibold text-slate-800">{formatMoney(o.total)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div>
            <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-500">Item-wise breakdown</h3>
            <div className="overflow-x-auto rounded-lg border border-slate-200">
              <table className="w-full text-left text-xs">
                <thead>
                  <tr className="border-b border-slate-100 bg-slate-50/70 text-[10.5px] uppercase tracking-wide text-slate-400">
                ...
                <tbody className="divide-y divide-slate-100">
                  {detail.items.map((it) => (
                    <tr key={`${it.sku}-${it.title}`}>
                      ...
                      <td className="px-3 py-2 text-right font-semibold text-slate-800">{it.quantity}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
  ...
  return (
    <div className="border-t border-slate-100 pt-3">
            ...
            {handovers === null ? (
              ...
              handovers.map((h) => (
                ...
                >
                  <span className="flex items-center gap-2 text-slate-600">
                    ...
                    <span
                      className={clsx(
                        ...
                        h.status === 'Confirmed' ? 'bg-emerald-50 text-emerald-700 ring-emerald-600/20' : 'bg-amber-50 text-amber-700 ring-amber-600/20'
                      )}
                    >
                      {h.status}
                    </span>
                  ...
                  <span className="flex items-center gap-2">
                    <span className="text-slate-400">{h.parcelCount} parcels</span>
                    <span className="font-semibold text-slate-800">{formatMoney(h.totalCodAmount)}</span>
                  </span>
                </button>
              ))
            )}
      ...
      <HandoverDetailModal
        ...
        onChanged={(updated) => setHandovers((prev) => prev?.map((h) => (h.id === updated.id ? updated : h)) ?? null)}
      />
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-1.5">
        ...
        <button onClick={handleCopy} className="shrink-0 rounded p-1 text-slate-400 hover:bg-slate-200 hover:text-slate-700">
    ...
    <div className="rounded-xl border border-slate-200 bg-white p-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-slate-800">{courier.displayName}</p>
            <p className="text-[11px] text-slate-300">
              {courier.lastUsedAt ? `Last used ${new Date(courier.lastUsedAt).toLocaleDateString()}` : 'Not used yet'}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <span
            className={clsx(
              ...
              courier.status === 'connected' ? 'bg-emerald-50 text-emerald-700 ring-emerald-600/20' : 'bg-rose-50 text-rose-700 ring-rose-600/20'
            )}
          >
            {courier.status === 'connected' ? 'Connected' : 'Error'}
          </span>
          <button onClick={() => onRemove(courier)} className="rounded-lg p-2 text-slate-400 hover:bg-rose-50 hover:text-rose-600">
            <Ban size={14} />
          </button>
        </div>
      </div>
      ...
      <div className="mt-4 space-y-2.5 border-t border-slate-100 pt-3">
        ...
        <p className="text-xs text-slate-400">
          Paste both into {meta.label}'s webhook settings so delivery status updates flow back into ZetSales automatically.
        </p>
        <button
...
export function CourierIntegrationsTab() {
  ...
  const handleConnected = (courier: CourierAccountDTO) => {
    setCouriers((prev) => [courier, ...prev.filter((c) => c.id !== courier.id)]);
  };
  ...
  const handleRegenerate = async (courier: CourierAccountDTO) => {
    try {
      const { courier: updated } = await regenerateCourierWebhookSecret(courier.id);
      setCouriers((prev) => prev.map((c) => (c.id === updated.id ? updated : c)));
      toast.push('New webhook secret generated — update it in your courier panel too.');
    } catch {
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="flex items-center justify-between rounded-xl border border-slate-200 bg-white p-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">{steadfastCourier ? 'Account connected' : 'Not connected'}</p>
            </div>
          </div>
          <button
        ...
        <div className="flex items-center justify-between rounded-xl border border-slate-200 bg-white p-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">{pathaoCourier ? 'Account connected' : 'Not connected'}</p>
            </div>
          </div>
          <button
      ...
      <div>
        <h2 className="mb-3 text-sm font-semibold text-slate-700">Connected couriers</h2>
        {loading ? (
          ...
          <div className="flex flex-col items-center gap-2 rounded-xl border border-dashed border-slate-300 py-14 text-center">
            ...
            <p className="text-sm font-medium text-slate-600">No couriers connected yet</p>
            <p className="text-xs text-slate-400">Connect Steadfast or Pathao above to start handing off shipped orders automatically.</p>
          </div>
        ) : (
          <div className="space-y-3">
            {couriers.map((courier) => (
              <CourierCard
                ...
                onChargeRateUpdated={(updated) => setCouriers((prev) => prev.map((c) => (c.id === updated.id ? updated : c)))}
              />
            ))}
          </div>
        )}
      ...
      <ConnectSteadfastModal open={steadfastModalOpen} onClose={() => setSteadfastModalOpen(false)} onConnected={handleConnected} />
      <ConnectPathaoModal open={pathaoModalOpen} onClose={() => setPathaoModalOpen(false)} onConnected={handleConnected} />
    </div>
  );
}
