...
export function DateTimePicker({
  ...
  const pickDay = (day: number) => {
    ...
    onChange(
      toValue(new Date(year, month, day, base.getHours(), base.getMinutes())),
    );
  };
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div
          className={clsx(
            "absolute z-20 w-64 zs-surface p-3 shadow-lg shadow-slate-900/10",
        ...
        >
          ...
          <div className="mt-3 flex items-center justify-center gap-3 border-t border-slate-100 pt-3">
            <TimeStepper
              label="Hour"
              value={pad(hour12)}
              onUp={() => commitTime(hour24 + 1, minute)}
              onDown={() => commitTime(hour24 - 1, minute)}
            />
            <span className="pt-3.5 text-sm font-semibold text-slate-400">
              :
            </span>
            <TimeStepper
              label="Min"
              value={pad(minute)}
              onUp={() => commitTime(hour24, minute + 5)}
              onDown={() => commitTime(hour24, minute - 5)}
            />
            <div className="flex flex-col items-center gap-1 pt-3.5">
              <button
                type="button"
                onClick={() =>
                  commitTime(ampm === "AM" ? hour24 + 12 : hour24 - 12, minute)
                }
...
function TimeStepper({
  ...
  return (
    <div className="flex flex-col items-center gap-1">
      ...
      <span className="w-8 rounded-md bg-slate-50 py-1 text-center text-sm font-semibold tabular-nums text-slate-700">
        {value}
      </span>
      <button
        type="button"
