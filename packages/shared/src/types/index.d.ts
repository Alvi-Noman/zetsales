...
export type OrderStage = 'Pending' | 'Flagged' | 'Confirmed' | 'Processing' | 'Shipped' | 'Out for Delivery' | 'Delivered' | 'Partial Delivered' | 'Returned' | 'Cancelled' | 'On Hold';
export type OrderPaymentStatus = 'COD Pending' | 'Advance Paid' | 'Paid' | 'Collected' | 'Refunded' | 'Failed';
export type HoldReason = 'Payment verification pending' | 'Address needs confirmation' | 'Stock check needed' | 'Customer requested reschedule' | 'Awaiting customer response' | 'Other';
export type CancelReason = 'Customer unreachable' | 'Customer changed mind' | 'Duplicate order' | 'Out of stock' | 'Fraud suspected' | 'Wrong address' | 'Price/payment dispute' | 'Other';
